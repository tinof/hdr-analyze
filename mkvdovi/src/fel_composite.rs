...
use crate::external::{
    self, run_command_live, run_command_with_progress, run_command_with_spinner,
};
use crate::metadata;
use crate::progress;
use crate::resume;
...
pub fn convert_fel_to_hdr10(input_file: &str, temp_dir: &Path, args: &Args) -> Result<PathBuf> {
    ...
    if resume_enabled && resume::is_complete(&composited_mkv) {
        progress::print_info("Reusing composited FEL output from a previous run.");
    ...
    if !composited_mkv.exists() || fs::metadata(&composited_mkv)?.len() == 0 {
        bail!("FEL encoder completed without producing a non-empty output");
    }
    resume::mark_done(&composited_mkv)?;
...
pub fn run_composite_pipe(pipe_args: &CompositePipeArgs) -> Result<()> {
    ...
    let temp_dir = std::env::temp_dir().join("mkvdovi-composite-pipe");
...
fn extract_hevc_from_mkv(
    ...
    if resume_enabled && resume::is_complete(output) {
        progress::print_info("Reusing extracted dual-layer HEVC from a previous run.");
        return Ok(());
    }
    ...
    if !run_command_with_progress(
        &mut cmd,
        &temp_dir.join("ffmpeg_extract_hevc.log"),
        "Extracting dual-layer HEVC stream",
        output,
        total,
        stall_timeout,
    )? {
