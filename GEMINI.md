...
- `--sample-rate <N>`: Analyze every Nth frame (default: 1). Skipped frames inherit measurements from previous analyzed frame.
...
- Performance optimizations:
  - **Frame Sampling**: Use `--sample-rate 3` to analyze every 3rd frame (massive speedup, minimal accuracy loss).
  - **Downscaling**: Use `--downscale 2` (default in mkvdolby fast mode) to reduce analysis resolution.
  - **Smart Skipping**: Scaling/cropping is skipped for non-analyzed frames.
- Expected performance: ~10-12 fps for 4K HEVC on 4-core Ampere instances (up to ~30-40 fps effective with sampling).
- Visual Progress: Shows progress bar, ETA, and processing speed on stderr.
