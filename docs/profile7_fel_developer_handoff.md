...
- There is a substantial Profile 7 handling path in `mkvdovi`.
...
Do not make this the default `mkvdovi` path.
...
- `mkvdovi/src/fel_composite.rs`
  ...
  - `apply_mmr_reshape()`
  - `composite_plane()`
- `mkvdovi/src/metadata.rs`
  - `HdrFormat::DolbyVisionFel`
  - `check_hdr_format()`
- `mkvdovi/src/pipeline.rs`
...
cargo test -p mkvdovi fel_composite -- --nocapture

# Current pixel-baked fallback path
target/release/mkvdovi "/path/to/p7_fel.mkv" --keep-source --fel-preset ultrafast -v
```
