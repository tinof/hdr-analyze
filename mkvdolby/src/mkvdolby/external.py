...
def run_ffmpeg_with_progress(
    ...
    try:
        with FfmpegProgress(command, exclude_progress=True) as ff:
            ...
            for pct in ff.run_command_with_progress(duration_override=duration_override):
                if pct is None:
                    continue
                if pct == 0 or pct == 100 or pct - last_print >= 1.0:
                    if pct == 100 and not finalizing_hint_shown:
                        print(f"\r{description}: {pct:6.2f}% (finalizing...)", end="", flush=True)
...
def check_dependencies():
    ...
    if not find_local_tool("dovi_tool"):
        print_color("red", "Error: Required command 'dovi_tool' not found in current directory or PATH.")
        all_found = False
