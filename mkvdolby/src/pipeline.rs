...
pub fn convert_file(input_file: &str, args: &Args) -> Result<bool> {
    ...
    if !input_path.exists() {
        println!("{}", format!("Warning: Input file not found: {}", input_file).yellow());
        return Ok(false);
    }
    ...
    if output_file.exists() {
         println!("{}", format!("Output file '{:?}' already exists. Skipping.", output_file).yellow());
         return Ok(true);
    }
    ...
    match hdr_type {
        ...
        HdrFormat::Hlg => {
            ...
            let mut extra_args = vec!["--hlg-peak-nits".to_string(), args.hlg_peak_nits.to_string()];
            add_optimizer_args(&mut extra_args, args);
            
            println!("{}", format!("HLG detected. Running analyzer natively with --hlg-peak-nits={}...", args.hlg_peak_nits).green());
            
            measurements_file = run_hdr_analyzer(input_file, &temp_dir, &extra_args)?;
            if measurements_file.is_none() { return Ok(false); }
        ...
        HdrFormat::Hdr10WithMeasurements | HdrFormat::Hdr10Unsupported => {
            ...
            if measurements_file.is_some() {
                 if args.boost_experimental {
                      println!("{}", "Experimental boost requested, but using existing measurements.".yellow());
                 }
                ...
                if args.boost_experimental {
                    println!("{}", "Experimental boost: using 'aggressive' optimizer.".green());
                    extra_args.extend(["--optimizer-profile".to_string(), "aggressive".to_string()]);
                } else {
                    add_optimizer_args(&mut extra_args, args);
                }
                measurements_file = run_hdr_analyzer(input_file, &temp_dir, &extra_args)?;
    ...
    let final_trims: Vec<u32> = args.trim_targets.split(',').filter_map(|s| s.trim().parse().ok()).collect();
    ...
    if rpu_path.is_none() { return Ok(false); }
    ...
    ffmpeg_cmd.args([
        ...
        "-i", bl_source_file.to_str().unwrap(),
        "-map", "0:v:0",
        "-c:v", "copy",
        "-f", "hevc",
        "-y",
    ...
    dovi_cmd.args([
        "inject-rpu",
        "-i", bl_hevc.to_str().unwrap(),
        "--rpu-in", rpu_path.to_str().unwrap(),
        "-o", bl_rpu_hevc.to_str().unwrap()
    ]);
    ...
    if args.drop_tags { mkvmerge_cmd.arg("--no-global-tags"); }
    if args.drop_chapters { mkvmerge_cmd.arg("--no-chapters"); }
    ...
    if args.verify {
        ...
        let ok = crate::verify::verify_post_mux(input_file, &output_file, measurements_file_path.as_deref(), &temp_dir);
        if !ok {
    ...
    println!("{}", format!("✓ Success! Created: {:?}", output_file.file_name().unwrap()).green().bold());
    Ok(true)
}
...
fn run_hdr_analyzer(input: &str, temp_dir: &Path, extra_args: &[String]) -> Result<Option<PathBuf>> {
...
fn extract_hdr10plus_metadata(input: &str, temp_dir: &Path) -> Result<Option<PathBuf>> {
    ...
    cmd.args(["-hide_banner", "-loglevel", "error", "-i", input, "-map", "0:v:0", "-c:v", "copy", "-f", "hevc", "-y", hevc.to_str().unwrap()]);
    ...
    tool.args(["extract", "-i", hevc.to_str().unwrap(), "-o", json_out.to_str().unwrap()]);
    
    if run_command(&mut tool, &temp_dir.join("hdr10plus_tool.log"))? {
        if json_out.exists() && fs::metadata(&json_out)?.len() > 0 {
...
fn convert_hlg_to_pq(input: &str, temp_dir: &Path, args: &Args) -> Result<PathBuf> {
    ...
    cmd.args([
        ...
        "-vf", &vf,
        "-c:v", "libx265",
        "-preset", &args.hlg_preset,
        "-crf", &args.hlg_crf.to_string(),
        "-pix_fmt", "yuv420p10le",
        "-profile:v", "main10",
...
fn generate_rpu(
    ...
    cmd.args(["generate", "-j", extra_json.to_str().unwrap(), "--rpu-out", rpu_out.to_str().unwrap()]);
    ...
    match hdr_type {
        HdrFormat::Hdr10Plus => {
             ...
             cmd.arg("--hdr10plus-peak-source").arg(peak_source.to_string());
        }
        HdrFormat::Hdr10WithMeasurements | HdrFormat::Hdr10Unsupported => {
